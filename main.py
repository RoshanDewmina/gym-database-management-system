...
def manageSchedules(name, memberID):
    ...
    if choice == "1":
        ...
        cursor.execute("SELECT FROM Training_Session WHERE SessionID =%s;", (sessionID,))
        ...
        classID = input("Enter ID of class to be cancelled: ")
        cursor.execute("SELECT FROM Registers WHERE ClassID =%s;", (classID,))
        status=cursor.fetchall()
        if(status):
